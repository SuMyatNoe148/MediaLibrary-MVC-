...
use MediaLibrary\Infrastructure\Persistence\NotificationRepository;
...
class NotificationService
{
    ...
    public function notifyReservationConfirmed(int $userId, string $mediaTitle): int
    {
        return $this->createNotification(
            ...
            "Your reservation for '{$mediaTitle}' has been confirmed.",
    ...
    public function notifyReservationCancelled(int $userId, string $mediaTitle): int
    {
        return $this->createNotification(
            ...
            "Your reservation for '{$mediaTitle}' has been cancelled.",
            'index.php?page=reservations'
        );
    }
    ...
    public function notifyPaymentCompleted(int $userId, string $invoiceNumber, float $amount): int
    {
        return $this->createNotification(
            ...
            'payment_completed',
            'Payment Completed',
            "Payment of $" . number_format($amount, 2) . " for invoice #{$invoiceNumber} was successful.",
    ...
    public function notifyNewReservation(int $adminId, string $userName, string $mediaTitle): int
    {
        return $this->createNotification(
            ...
            'new_reservation',
            'New Reservation',
            "{$userName} has made a new reservation for '{$mediaTitle}'.",
            'index.php?page=admin-reservations'
        );
    }
    ...
    public function notifyNewPayment(int $adminId, string $userName, float $amount): int
    {
        return $this->createNotification(
            ...
            'new_payment',
            'New Payment',
            "{$userName} has made a payment of $" . number_format($amount, 2) . ".",
            'index.php?page=admin-invoices'
        );
    }

    public function notifyInvoiceDownloaded(int $userId, string $invoiceNumber): int
    {
        return $this->createNotification(
            $userId,
            'invoice_downloaded',
            'Invoice Downloaded',
            "You have downloaded invoice #{$invoiceNumber}.",
            'index.php?page=invoices'
        );
    }
}
